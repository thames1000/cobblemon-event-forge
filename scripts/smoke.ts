...
// === Event import/export (portable config) ===
import { toPortableEvent, fromPortableEvent, normalizeEventConfig, EVENT_PORTABLE_TYPE } from "../src/lib/event/portable";

// the generated bundle now embeds a re-importable snapshot
const ecSidecar = bundle.files.find((f) => f.path === "event_config.json");
if (!ecSidecar) errors.push("portable: event bundle missing event_config.json side-car");
if (ecSidecar) {
  const parsed = JSON.parse(ecSidecar.contents);
  if (parsed._type !== EVENT_PORTABLE_TYPE) errors.push("portable: side-car missing the type marker");
  // round-trip: re-import the side-car and regenerate → identical datapack files
  const reimported = fromPortableEvent(ecSidecar.contents);
  const regen = generateEvent(reimported);
  const norm = (b: typeof bundle) => b.files.filter((f) => f.path !== "event_config.json").map((f) => f.path + "::" + f.contents).sort().join("\n");
  if (norm(regen.bundle) !== norm(bundle)) errors.push("portable: re-imported config does not regenerate an identical event");
  if (reimported.title !== cfg.title) errors.push("portable: round-trip lost the title");
  if (reimported.rewardTiers.find((t) => t.id === "winner")?.award !== "completion-each") errors.push("portable: round-trip lost a tier's award mode");
}
// fromPortableEvent accepts a BARE EventConfig too (not just the wrapped form)
const bare = fromPortableEvent(JSON.stringify(cfg));
if (bare.title !== cfg.title) errors.push("portable: bare-config import failed");
// normalize fills defaults for a partial / older-schema config without crashing
const partial = normalizeEventConfig({ title: "Skeletal Import", objectives: [{ triggerId: "cobblemon:catch_pokemon", count: 7 }], rewardTiers: [{ name: "Winner" }] });
if (partial.title !== "Skeletal Import") errors.push("portable: normalize dropped the title");
if (partial.presetId !== "blank") errors.push("portable: normalize should fall back to the blank preset for unknown/missing presetId");
if (partial.objectives[0].mode !== "manual" || partial.objectives[0].id !== "o1" || !Array.isArray(partial.objectives[0].rewards)) errors.push("portable: normalize didn't backfill objective defaults");
if (typeof partial.pack.includeUninstall !== "boolean" || typeof partial.packFormat !== "number") errors.push("portable: normalize didn't backfill pack/packFormat");
if (partial.rewardTiers[0].id !== "tier1" || !Array.isArray(partial.rewardTiers[0].actions)) errors.push("portable: normalize didn't backfill a tier missing id/actions");
// a config generated by normalize is still generatable
if (!generateEvent(partial).validation.ok) errors.push("portable: normalized partial config produced an invalid datapack");
// bad JSON throws (caught by the UI)
let threw = false;
try { fromPortableEvent("{not json"); } catch { threw = true; }
if (!threw) errors.push("portable: fromPortableEvent should throw on bad JSON");
